...
# peer lifecycle chaincode approveformyorg --package-id $CCID --channelID soshchannel --name products --version 1 --sequence $2 --waitForEvent --tls --cafile $ORDERER_CA
# peer lifecycle chaincode checkcommitreadiness --channelID soshchannel --name products --version 1  --sequence $2 --tls --cafile $ORDERER_CA
...
# peer lifecycle chaincode commit -o orderer.sosh.com:7050 --channelID soshchannel --name products --version 1 --sequence $2 --tls true --cafile $ORDERER_CA --peerAddresses peer0.superadmin.sosh.com:7051 --peerAddresses peer0.users.sosh.com:9051  --tlsRootCertFiles ./crypto/peerOrganizations/superadmin.sosh.com/peers/peer0.superadmin.sosh.com/tls/ca.crt --tlsRootCertFiles ./crypto/peerOrganizations/users.sosh.com/peers/peer0.users.sosh.com/tls/ca.crt
